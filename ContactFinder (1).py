...
epatterns.append('([a-zA-Z]+)\sat\s([a-zA-Z]+)\s([a-zA-Z]+)\s+edu')
epatterns.append('email: ([A-Za-z.]+) at ([A-Za-z.]+) ([A-Za-z.]+) edu')
epatterns.append('([A-Za-z]+)\sat\s([A-Za-z]+)\sdot\s([A-Za-z]+)\sdot\s([A-Za-z]+)')
...
#ppatterns.append('\+?\d{1,3}\s*-\s*(\d{3})-(\d{3})-(\d{4})')
##ppatterns.append('(\d{3})-?(\d{3})-?(\d{4})')
ppatterns.append('\(?(\d{3})\)?\s*-?\s*(\d{3})-(\d{4})')
ppatterns.append('(\d{3})\s(\d{3})-(\d{4})')
ppatterns.append('\[(\d{3})\]\s(\d{3})-(\d{4})')
#ppatterns.append('\((\d{3})\)\s(\d{3})-(\d{4})')
    ...
    for line in f:
        ...
        for epat in epatterns:
            ...
            for m in matches:
                ...
                if epat.endswith("com"):
                    email = '{}@{}.com'.format(m[0], m[1])
                elif (" at " in line and " dot " not in line) and len(m)==3:  # Identify pattern without explicit 'dot'
                    email = '{}@{}.{}.edu'.format(m[0], m[1], m[2])
                elif len(m) == 4:
                    y = '{}.{}'.format(m[1], m[2])
                    email = '{}@{}.edu'.format(m[0], y)   
                else:
